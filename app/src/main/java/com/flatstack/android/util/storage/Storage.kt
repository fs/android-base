...
class Storage(private val sp: SharedPreferences, private val gson: Gson) : IStorage {
    ...
    override fun put(key: String, items: Any) = sp.edit().putString(key, gson.toJson(items)).apply()

    override fun putString(key: String, str: String) = sp.edit().putString(key, str).apply()

    override fun getString(key: String): String? = sp.getString(key, null)

    override fun putLong(key: String, number: Long) = sp.edit().putLong(key, number).apply()

    override fun getLong(key: String, defaultValue: Long) = sp.getLong(key, defaultValue)

    override fun putInt(key: String, number: Int) = sp.edit().putInt(key, number).apply()
